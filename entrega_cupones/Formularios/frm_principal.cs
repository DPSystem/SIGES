...
namespace entrega_cupones
{
  public partial class frm_principal : Form
  {
    ...
    public frm_principal(int id_usuario, string user_name, string dni, string rol, int rolID)
    {
      ...
      var foto = buscar.get_titular(dni)  ;
     
      user_nombre = user_name;

      if (foto.foto != null)
      {
        Picture_box_user.Image = conv_img.ByteArrayToImage(foto.foto.ToArray());
      }
    ...
    private void activar_usuarios_objetos(string rol, int rolID)
    {
      ...
      //  btn_masajista.Enabled = true;
      //  btn_pedicuro.Enabled = true;
      //}
      //else
      //{
      //  btn_ActivarSocio.Visible = false;
      //  btn_masajista.Enabled = false;
      //}

      //if (user_id == 8 || user_id == 9 ) // si es daniel diaz
      //{
      //  btn_inspectores.Enabled = true;
      //  btn_pedicuro.Enabled = false;
      //  btn_colonia.Enabled = false;
      //  btn_masajista.Enabled = true;
      //}
      //else
      //{
      //  //btn_inspectores.Enabled = false;
      //  btn_pedicuro.Enabled = true;
      //  btn_colonia.Enabled = false;
      //}
      usuarios user = new usuarios();
      var permisos = user.get_permisos(user_id); // obtengo la lista de los permisos de un usuario
    ...
    private void btn_mochila_Click(object sender, EventArgs e)
    {
      ...
      f_mochilas._cuil = Convert.ToDouble(lbl_cuil.Text);
      f_mochilas.UsuarioID = user_id;
    ...
    private void btn_quinchos_Click(object sender, EventArgs e)
    {
      ...
      {
        Frm_Quinchos2 f_quinchos = new Frm_Quinchos2();
    ...
    private void btn_masajista_Click(object sender, EventArgs e)
    {
      ...
      if (user_id == 3 || user_id == 8 || user_id == 27 || user_id == 4 || user_id == 9)
      {
        if (MessageBox.Show("Ingresar como cobrador??", "¡¡¡ ATENCION - Seleccione la Opcion Deseada!!!", MessageBoxButtons.YesNo) == DialogResult.Yes)
        {
          frm_CobranzasAsignadas f_cobranzasAsig = new frm_CobranzasAsignadas();
          f_cobranzasAsig.CobradorId = user_id;
          f_cobranzasAsig.CobradorNombre = user_nombre;
          f_cobranzasAsig.Show();
        }
        else
        {
          frm_cobranzas f_cobranzas = new frm_cobranzas();
          f_cobranzas.UserId = user_id;
          f_cobranzas.Show();
        }
      }
      else
    ...
    private string obtener_localidad(int codloc)
    {
      ...
      using (var context = new lts_sindicatoDataContext())
      {
        if (codloc != 0)
        {
          ...
          if (loc.Count() > 0)
          {
            localidad = loc.First().MAELOC_NOMBRE.Trim();
          }
    ...
    {
      frm_Principal2 f_principal2 = new frm_Principal2(user_id, user_nombre,DNI, rol_nombre, user_rol);
      f_principal2._UserId = user_id;
      f_principal2._UserDNI = DNI;
      f_principal2._UserRol = user_rol;
      f_principal2._UserNombre = user_nombre;
      f_principal2._RolNombre = rol_nombre;
      f_principal2.ShowDialog();
    ...
    private void btn_gimnasio_Click(object sender, EventArgs e)
    {
      frm_Actas2 f_Actas2 = new frm_Actas2();
      f_Actas2.ShowDialog();
    }
    ...
    private void btn_EntregarMochila_Click(object sender, EventArgs e)
    {
      ...
      f_EntregaMochila.UsuarioId = user_id;
      f_EntregaMochila.UsuarioNombre = user_nombre;
