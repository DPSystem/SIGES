...
namespace entrega_cupones
{
  public partial class frm_inspecciones : Form
  {
    ...
    private void frm_inspecciones_Load(object sender, EventArgs e)
    {
      ...
      var localidad = from loc in db_socios.localidades
                      where loc.idprovincias == 14
                      select loc;
      ...
      foreach (var em in empresa.ToList())
      {
        ...
        var actas_involucradas = from act in db_socios.ACTAS
                                 ...
                                 select new
                                 {
                                   ...
                                   inspector = act.INSPECTOR,
                                   importe_acta = act.DEUDATOTAL
                                   //ultimoPeriodo =  UltPer.obtener_periodo(em.cuitstr).Year == 1900 ? "-" : UltPer.obtener_periodo(em.cuitstr).Date.ToString(),
                                 };
        ...
        dgv_ranking.Rows[dgv_ranking.Rows.Count - 1].Cells["empresa"].Value = em.Nombre.Trim();
        dgv_ranking.Rows[dgv_ranking.Rows.Count - 1].Cells["domicilio"].Value = em.domicilio.Trim();
        dgv_ranking.Rows[dgv_ranking.Rows.Count - 1].Cells["deuda"].Value = deuda_empresa;
        dgv_ranking.Rows[dgv_ranking.Rows.Count - 1].Cells["ultimoPeriodo"].Value = UltPer.obtener_periodo(em.cuitstr).Year == 1900 ? "-" : UltPer.obtener_periodo(em.cuitstr).Date.ToString("d");
    ...
    private void btn_per_NO_declarados_Click(object sender, EventArgs e)
    {
      ...
      var periodo_declarado = (from a in db_socios.ddjj
                               where (a.periodo >= desde && a.periodo <= hasta) && (a.cuite == num_cuit)// && (a.cuil == 27311202761))
